...
cur_address:
	...
	beq	cur_addr_default	; Is it end of buffer (0)? Yes, so use default address (CUR_ADDR)
	cmp	#" "			; Is it the space delimiter?
	beq	cur_addr_get		; Yes it is. Then go read an address
	jsr	syntax_error		; If anything else, print a syntax error
	rts				; End of routine

cur_addr_default:			; IF NO ADDRESS PROVIDED, GET CUR ADDRESS USED +1
...
cur_addr_get:				; GET START ADDRESS
	...
	bcc	cur_addr_error		; Error in address
...
clock:				
	...
	lda	IN_BUFFER,y		; Read a character from the input buffer
	beq	read_time		; Yes it is. Since nothing is specified, print time
	cmp	#" "			; Is it the space delimiter?
	bne	clock_error		
	jmp	set_time		; Yes it is. Then go read an address
clock_error:
	jsr	syntax_error		; If anything else, print a syntax error
	rts
...
set_time_help:
	jsr	error_pointer
	ldx	#<time_error	; Get LSB address
	ldy	#>time_error	; Get MSB address
...
find_validate:
	ldx	#<find_validate_msg	; Get LSB address of msg
	ldy	#>find_validate_msg	; Get MSB address of msg
	jsr	print_string		; Write  message
...
; Magic number - fills RAM with a sequence of several bytes
; ----------------------------------------------------------------------------------------------------------------------
...
io_map:
	ldx	#<io_map_msg		; Get LSB address of msg
	ldy	#>io_map_msg		; Get MSB address of msg
	jsr	print_string		; Write message
	rts	
...
; Send a stream of bytes to a specific address
; ---------------------------------------------------------------------------------------------------------------------------------
...
invalid_byte:
	jsr	error_pointer		; Print error pointer underneath the command-line culprit
	ldx	#<invalid_byte_msg	; Get LSB address of msg
	ldy	#>invalid_byte_msg	; Get MSB address of msg
	jsr	print_string		; Write message
	rts
